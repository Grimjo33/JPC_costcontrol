document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadReport(params = {}) {
        ...
        const url = new URL('/api/bpp-hauling-report', window.location.origin);
        ...
        try {
            ...
            // Render bagian ringkasan
            document.getElementById('summary-total-biaya').textContent = formatCurrency(data.summary.totalBiayaProduksi);
            document.getElementById('summary-total-tonase').textContent = `${formatNumber(data.summary.totalTonaseHauling)} Ton`;
            document.getElementById('summary-bpp').textContent = `${formatCurrency(data.summary.bppHauling)} / Ton`;

            // Render bagian rincian & grafik
            renderCostBreakdown(data.costBreakdown, data.summary.totalBiayaProduksi, data.summary.totalTonaseHauling);
    ...
    function renderCostBreakdown(costData, total, totalTonase) { // Tambahkan parameter totalTonase
    ...
    costData.forEach(item => {
        ...
        // Hitung biaya per ton untuk setiap komponen
